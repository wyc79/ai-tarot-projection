...
- The graph row `#graph-row` is `calc(100dvh - 4rem)` tall with `scroll-margin-top: 2rem` and its row `minmax(0, 1fr)`; a scenario click scrolls it into view (`block: "start"`, smooth); below `64rem` it stacks, the frame `60dvh` and `#scenarios` `max-height: 16rem`. (Task 2 was written and reviewed against a fixed `--picture-height: calc(100vh - 6rem)`; the hand check and then the human's look replaced it — see the spec's Layout section and the changelog. `graph.html`, `graph.css` and `graph-page.js` as committed are the truth, not Task 2's blocks: there is no `--picture-height`, the hint lives in the zoom bar, the wrapper class is `frame`, and the click handler scrolls.)
