/**
 * Protocol scanner: reads a session and reports where the reader broke its own
 * rules. No DOM, no fetch, no node -- the command line that runs it over files
 * is scripts/scan.mjs, and it is the thin half.
 *
 * It lives here rather than in scripts/ because it is not a script. Its callers
 * are the engine tests, the seeded fixture, the A/B harness and the debug page,
 * so one definition of "a violation" covers every way a transcript gets
 * produced -- and it reads the same session record the engine writes, using the
 * same questionType and the same ladder arithmetic. A check that drifted from
 * those would be checking a protocol nothing implements.
 *
...
import { levelDistance, levelIndex } from "./levels.js";
import {
...
/**
 * The session inside whatever was handed over.
 *
 * Three shapes reach the scanner: a journal export {session}, a bare session,
 * and whatever someone pasted out of localStorage, which the storage module
 * wraps as {v, data}. Reading it wrong used to arrive as a TypeError about
 * reading 'map' of undefined, which says nothing about the file.
 *
 * Here rather than in the command line because the debug page takes pasted
 * sessions too, and "what counts as a session" is the scanner's question.
 */
export function sessionIn(parsed) {
  const session = parsed?.session ?? parsed?.data ?? parsed;
  return Array.isArray(session?.exchanges) ? session : null;
}
